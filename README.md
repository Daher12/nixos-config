...
|------|----------|------|------------|
| **yoga** | Lenovo Yoga 7 Slim Gen 8 (AMD Ryzen) | Primary laptop | GNOME 50 · Secure Boot (Lanzaboote) · LUKS + Btrfs impermanence · Windows 11 VM · ZRAM |
| **latitude** | Dell Latitude E7450 (Intel) | Legacy laptop | GNOME · TLP · preload-ng · ext4 |
...
- **Desktop:** GNOME 50 (Wayland), auto-login, Colloid-Nord GTK theme, Fluent
  icons, Posy cursors, automatic dark/light switching via darkman.
...
- **Virtualization:** libvirt/QEMU with virt-manager, SPICE USB redirection,
  and a Windows 11 launcher with DHCP-reserved IP.
...
classic ext4 + LUKS without impermanence. Intel-specific tuning (i915
...
├── profiles/            # role bundles: laptop, desktop-gnome
...
└── .github/workflows/   # daily input bump + lint CI, opencode PR bot
```
...
- **CI** (`.github/workflows/bump.yml`): daily at 02:00 UTC — update the safe
  flake inputs → check → dry-build all three hosts → format → auto-commit.
  Locked inputs (lanzaboote, opencode) only move on explicit request.
...
The installer auto-detects the host's needs from the config: Disko
partitioning when `hosts/<host>/disks.nix` exists, the impermanence
`@blank` template snapshot for Btrfs hosts, SSH host-key / machine-id
restoration from an optional backup USB path, password-hash provisioning,
and post-install verification (root not locked, bootloader present).
