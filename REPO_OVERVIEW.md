...
- **Hibernate-bridge law (post-mortem 2026-09-08, eight attempts):** hibernating **directly from a running session always aborts** in the kernel freeze pass — consistently surfacing as `usb 3-3: WARN: invalid context state for evaluate context command` with a deep amdgpu unwind — while hibernating **right after waking from s2idle works** (the 04:00 cycle: RTC wake → 3s → clean image + poweroff). The USB surface is a red herring: every manipulation of usb 3-3 / its xHCI controller was tried and changed nothing — bound+active, interface-unbound, device-unbound (aborts differently: `usb_dev_restore -107`), bound+runtime-suspended via rfkill, `authorized 0→1` re-enumeration, and a full `xhci_hcd` driver rebind of controller `0000:04:00.4` (all 2026-09-07/08; the rebind demonstrably executed and the WARN persisted). Whatever goes stale is cleared by a real suspend/resume cycle but not by any USB-level re-init. **Fix: never hibernate from a running state.** The unattended deadline path already hibernates right after an RTC wake (unbroken since). For manual hibernates, `systemd-hibernate.service` is overridden in `hosts/yoga/default.nix`: no bridge-done flag → set `/run/yoga-s2h-bridge` and suspend with a 10s RTC self-wake (armed by the hook's `pre:suspend`); the hook's `post:suspend` then sets bridge-done and re-requests hibernate through logind, which lands in the overridden unit again and runs the stock `systemd-sleep hibernate` directly. Net cost ~15s per manual hibernate. All the USB quiesce/rebind machinery was removed — leave the USB stack alone.
