...
- **8087:0032 reset-before-hibernate (converged 2026-09-07 23:19):** the AX210 Bluetooth USB device (`usb 3-3`) breaks hibernate unless it has been **reset since boot** — its xHC slot comes up in a state that trips the freeze pass (`usb 3-3: WARN: invalid context state for evaluate context command`) and aborts the hibernate mid-flight; the abort class that can corrupt amdgpu/TTM state. Five attempts: bound+active → WARN abort (23:38, 22:02); interface-unbound → still WARN abort (22:02 variant); fully unbound → no WARN but `usb_dev_restore -107` abort (22:36); **bound + runtime-suspended (rfkill quiesce) → STILL WARN abort (23:19, `8087:0032 at freeze: suspended` in the hook log)**; the only clean success (04:00) came after s2idle cycles had reset the device (`usb 3-3: reset ... device number N` on every resume). The hook therefore replicates that reset in `pre:hibernate` via the `authorized` 0→1 toggle (deconfigure + re-enumerate like a replug, staying bound), followed by rfkill block + autosuspend-wait for quiescence; `post:hibernate` unblocks rfkill. Diagnostic line in the journal: `yoga-s2h: 8087:0032 re-enumerated ...` / `... at freeze: suspended`. Plain suspend is unaffected (suspend, not freeze callbacks).
