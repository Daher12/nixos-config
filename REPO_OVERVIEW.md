...
- **xHCI controller rebind before hibernate (converged 2026-09-07 23:29):** the AX210 Bluetooth USB device (`usb 3-3`) breaks hibernate unless the **xHCI controller** hosting it has been re-initialized since boot — the freeze pass dies with `usb 3-3: WARN: invalid context state for evaluate context command` and the hibernate aborts mid-flight; the abort class that can corrupt amdgpu/TTM state. Six attempts: bound+active → WARN abort (23:38, 22:02); interface-unbound → still WARN abort; device-unbound → no WARN but `usb_dev_restore -107` abort (22:36); bound+runtime-suspended (rfkill quiesce, 23:19) → WARN abort; device-level `authorized 0→1` re-enumeration (23:29) → **still WARN abort** — so the bad state is controller-side, not device-side. The only clean success (04:00) followed s2idle suspend/resume cycles of the controller. The hook therefore rebinds the `xhci_hcd` driver for the controller hosting 8087:0032 (`unbind; sleep 2; bind`, discovered by walking the device's sysfs path; on this machine usb3 carries only the BT controller, so the blast radius is BT alone), waits for re-enumeration, then rfkill-blocks and waits for runtime suspend. `post:hibernate` unblocks rfkill. Journal markers: `yoga-s2h: xhci … rebound` and `… at freeze: suspended`. Plain suspend is unaffected (suspend, not freeze callbacks).
